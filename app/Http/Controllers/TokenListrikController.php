...
class TokenListrikController extends Controller
{
    public function index()
    {
        ...
        try {
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                return view('user.token', compact('data'));
            }
    ...
    public function Store(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/token_pln', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                ],
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                ...
                return redirect()->back()->with('success', 'Token listrik ditemukan.')->with('data', $data['data']);
            }

            return redirect()->back()->with('error', $data['message'] ?? 'Gagal mengambil data token listrik.');
        } catch (\Exception $e) {
    ...
    public function transaction(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/token_pln/data', [
                'form_params' => [
