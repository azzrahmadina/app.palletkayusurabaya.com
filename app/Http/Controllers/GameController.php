...
class GameController extends Controller
{
    public function index()
    {
        ...
        try {
            ...
            $gameResponse = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/get_type_games', [
                'form_params' => [
                    'token' => $token,
                ]
            ...
            $gameData = json_decode($gameResponse->getBody(), true);


            if (!isset($gameData['data']) || !is_array($gameData['data'])) {
    ...
    public function Store(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/games', [
                'form_params' => [
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                ...
                return redirect()->back()->with('success', 'TGames ditemukan.')->with('data', $data['data']);
            }

            return redirect()->back()->with('error', $data['message'] ?? 'Gagal mengambil data Games.');
        } catch (\Exception $e) {
    ...
    public function transaction(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/games/data', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                    'code' => $request->game_code,
