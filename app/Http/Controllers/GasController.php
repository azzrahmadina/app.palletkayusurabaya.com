...
class GasController extends Controller
{
    public function index()
    {
        ...
        try {
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                return view('user.gas', compact('data'));
            }
    ...
    public function Store(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/pgas_pascabayar', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                ],
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                ...
                return redirect()->back()->with('success', 'Tagihan listrik ditemukan.')->with('data', $data['data']);
            }

            return redirect()->back()->with('error', $data['message'] ?? 'Gagal mengambil data tagihan listrik.');
        } catch (\Exception $e) {
    ...
    public function transaction(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/pgas_pascabayar/data', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                ],
