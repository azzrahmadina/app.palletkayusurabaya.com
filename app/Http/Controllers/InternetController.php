...
class InternetController extends Controller
{
    public function index()
    {
        ...
        try {
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                return view('user.internet', compact('data'));
            }
    ...
    public function Store(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/internet_pascabayar', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                ],
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                ...
                return redirect()->back()->with('success', 'Tagihan internet ditemukan.')->with('data', $data['data']);
            }

            return redirect()->back()->with('error', $data['message'] ?? 'Gagal mengambil data tagihan internet.');
        } catch (\Exception $e) {
    ...
    public function transaction(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/internet_pascabayar/data', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                ],
