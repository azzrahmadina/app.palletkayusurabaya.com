...
class BpjsController extends Controller
{
    public function index()
    {
        ...
        try {
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                return view('user.bpjs', compact('data'));
            }
    ...
    public function Store(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/bpjs_pascabayar', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                ],
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                ...
                return redirect()->back()->with('success', 'Tagihan BPJS ditemukan.')->with('data', $data['data']);
            }

            return redirect()->back()->with('error', $data['message'] ?? 'Gagal mengambil data tagihan BPJS.');
        } catch (\Exception $e) {
    ...
    public function transaction(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/bpjs_pascabayar/data', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                ],
