...
class PbbController extends Controller
{
    public function index()
    {
        ...
        try {
            ...
            $locationResponse = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/get_type_pbb_pascabayar', [
                'form_params' => [
                    'token' => $token,
                ]
    ...
    public function Store(Request $request)
    {
        ...
        try {
            ...
            if (isset($data['status']) && $data['status'] === 'sukses') {
                ...
                return redirect()->back()->with('success', 'Tagihan PBB ditemukan.')->with('data', $data['data']);
            }

            return redirect()->back()->with('error', $data['message'] ?? 'Gagal mengambil data tagihan PBB.');
        } catch (\Exception $e) {
    ...
    public function transaction(Request $request)
    {
        ...
        try {
            ...
            $response = $client->post('https://bospulsa.wahyurosamhidayat.xyz/api/pbb_pascabayar/data', [
                'form_params' => [
                    'nomor' => $request->nomor,
                    'token' => $token,
                    'code' => $request->location_code,
